...
#Region "Contructor"
    ...
    Public Sub GENERAR_ALBARANS_TLY()
        Dim ls As String
        ...
        '
        'Consutla Capçalera de factures
        'ls = ""
        'ls = ls & " SELECT"
        ...
        'ls = ls & " AND ISNULL(T0.U_SEIFiVox,'')=''"    ' albaran no exportada a Voxel   
        '


        'Consutla Capçalera d'Albarans
        ls = ""
        ls = ls & " SELECT"
        ls = ls & " T0.CardCode,  T0.DocNum,     T0.DocEntry,  T1.U_SEIEdiC,"
        ls = ls & " T0.DocDate,   T0.DocDueDate, T0.U_SEI_EDIR, "
        ...
        ls = ls & " LEFT OUTER JOIN OCTG T2"
        ls = ls & " ON T0.GroupNum= T2.GroupNum "
        ls = ls & " WHERE T1.U_SEIAlbVoxel = 'Y' "         ' Cliente con Flag Facturas VOXEL
        ls = ls & " AND ISNULL(T0.U_SEIfivoxAl ,'')=''"    ' albaran no exportada a Voxel   
        '
        ''    ls = ls & " AND ISNULL(T0.DocNum ,'')='7490428' "
        ...
        Try
            ...
            Do While Not oRecordset.EOF
                ...
                ls = ""
                If esAngrup Then
                    ''' aquesta era la consulta per les factures 
                    'ls = ls & "  select  baseref, baseentry , t2.U_SEINUMPE  FROM  OINV T0 "
                    'ls = ls & "  INNER JOIN  INV1 T1  ON T0.DocEntry=T1.DocEntry  "
                    'ls = ls & "  left join ODLN t2 on T1.BaseEntry = t2.docentry "
                    'ls = ls & "   where(T0.DocEntry = " & oRecordset.Fields.Item("DocEntry").Value.ToString & ")" '''oDataReader("DocEntry").ToString
                    'ls = ls & "  group by baseref, baseentry, t2.U_SEINUMPE "
                    ''' fi de la consulta per a les factures
                    ''' 

                    ls = ls & "  select t0.DocNum  , t1.DocEntry , t0.U_SEINUMPE, t0.NumAtCard  FROM  ODLN T0 "
                    ls = ls & "  INNER JOIN  DLN1 T1  ON T0.DocEntry=T1.DocEntry  "
                    ls = ls & "   where(T0.DocEntry = " & oRecordset.Fields.Item("DocEntry").Value.ToString & ")" '''oDataReader("DocEntry").ToString
                    ls = ls & "  group by t0.DocNum  , t1.DocEntry , t0.U_SEINUMPE, t0.NumAtCard  "
                ...
                Else
                    ...
                    ls = ls & "  select top 1 DocNum  FROM  ODLN T0 "
                    ls = ls & "  INNER JOIN  DLN1 T1  ON T0.DocEntry=T1.DocEntry  "
                    ls = ls & "   where(T0.DocEntry = " & oRecordset.Fields.Item("DocEntry").Value.ToString & ")" '''oDataReader("DocEntry").ToString
                    ls = ls & "  group by DocNum,NumAtCard "
                End If
                ...
                If esAngrup Then
                    ...
                    While oDataReader2.Read()
                        ...
                        If Not IsNothing(oDataReader2("U_SEINUMPE")) And oDataReader2("U_SEINUMPE").ToString <> "" Then
                            ...
                            End If
                        Else
                            If Not IsNothing(oDataReader2("NumAtCard")) And oDataReader2("NumAtCard").ToString <> "" Then
                                If Not referenciesComandes.ContainsKey(oDataReader2("NumAtCard").ToString) Then
                                    If xLinia > 0 Then
                                        oDocumentLines = oXml.SelectSingleNode("//References")
                                        oFirstRow = oDocumentLines.FirstChild
                                        oNewRow = oFirstRow.CloneNode(True)
                                    ...
                                    referenciesComandes.Add(oDataReader2("U_SEINUMPE").ToString, oDataReader2("NumAtCard").ToString)
                                    xLinia = xLinia + 1
                                End If
                            End If
                    ...
                    While oDataReader2.Read()
                        ...
                        ''' això és per albarans
                        If Not IsNothing(oDataReader2("DocNum")) And oDataReader2("DocNum").ToString <> "" Then
                            If Not referenciesAlbarans.ContainsKey(oDataReader2("DocNum").ToString) Then
                                If xLinia > 0 Then
                                    oDocumentLines = oXml.SelectSingleNode("//References")
                                    oFirstRow = oDocumentLines.FirstChild
                                    oNewRow = oFirstRow.CloneNode(True)
                                ...
                                End If
                                oItem = oXml.SelectNodes("//References/Reference")
                                oItem.Item(xLinia).Attributes("DNRef").InnerText = oDataReader2("DocNum").ToString
                                oItem.Item(xLinia).Attributes("PORef").InnerText = ""
                                referenciesAlbarans.Add(oDataReader2("DocNum").ToString, oDataReader2("DocNum").ToString)
                                xLinia = xLinia + 1
                            End If
                        End If
                        '''' di de albarans
                ...
                Else
                    ...
                    While oDataReader2.Read()
                        ...
                        End If
                        oItem = oXml.SelectNodes("//References/Reference")
                        If IsNothing(oDataReader2("DocNum")) Or oDataReader2("DocNum").ToString = "" Then
                        ...
                        Else
                            If Trim(oRecordset.Fields.Item("U_SEINUMPE").Value.ToString) <> "" Then
                ...
                go_conn3.Close()
                Me.Form.lblmsg.Text = "Albaran: " & oRecordset.Fields.Item("CardCode").Value.ToString ''' oDataReader("CardCode").ToString
                '''''''''''''''-------------------------------------------------------------------------------------------
                ...
                sFicheroD = sPathD & "\" & "Albaran_" & oRecordset.Fields.Item("DocNum").Value.ToString & "_" & "000" & ".xml"   '''oDataReader("DocNum").ToString
                '
                oXml.Save(sFichero)
                oXml.Save(sFicheroD)
                '
                ...
                ls = ls & " update  ODLN  set U_SEIfivoxAl = '" & sFichero & "'where docentry  = " & oRecordset.Fields.Item("DocEntry").Value.ToString
                '' ''oDataReader = oSqlcomand.ExecuteReader()
                orecordset2 = Nothing
            ...
            While oEnumerador.MoveNext
                ...
                ls = " update  ODLN  set U_SEIfivoxAl = '" & oEnumerador.Value & "'where docentry  = " & oEnumerador.Key
                SEI_SRV_VOXEL.ConectarSQLNative(go_conn3)
                oSqlcomand = Nothing
                oSqlcomand = New SqlCommand(ls, go_conn3)
    ...
    Private Function ObtenerRcsTiposIvas(ByVal lDocEntry As Long) As ADODB.Recordset
        ...
        Dim sCab As String = "ODLN"
        Dim sDet As String = "DLN1"
